...
/**
 * Function login user based on the filter provided.
 * @param  {Object} filter Json object contains filter data.
 * @param  {Object} bodyData Json object contains data which need to update.
 */
exports.loginUserDetails = async (filter, bodyData) => {
	try {
	
		let token = await User.jwt.sign(bodyData, process.env.SECRET_KEY,  {
			expiresIn: '24h',
			where: filter
		});
